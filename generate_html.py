...
print("Starting HTML extraction and generation...")

with open('output.json', 'r', encoding='gbk', errors='ignore') as f:
    data = f.read()

# The real JSON output from Gemini starts with either '[\n  {' or '[{\n'
# Let's find it.
start_idx = data.find('[\n  {\n    "type"')
if start_idx == -1:
    start_idx = data.find('[\n  {')
...
if start_idx == -1 or end_idx == 0 or end_idx <= start_idx:
    print("FATAL: Could not find valid JSON array bounds in output.json")
    print(data[:500])
    exit(1)

raw_json = data[start_idx:end_idx]

# There could be corrupted bits if standard out got interleaved. Let's try to parse.
try:
    slides_json = json.loads(raw_json)
    print(f"Successfully loaded {len(slides_json)} slides.")
except Exception as e:
    print("Failed to parse the exact extracted block:", e)
    # let's write to a debug file and exit
    with open('debug_extracted.json', 'w', encoding='utf-8') as fdebug:
        fdebug.write(raw_json)
    exit(1)
